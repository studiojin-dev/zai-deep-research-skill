...
- `memory_db_path`: 반복 요약, 보고서, 아티팩트, lexical memory 인덱스를 저장하는 SQLite 데이터베이스
...
- `--validate --json` 은 validation 상태, 감지한 MCP 이름, 누락 MCP, lexical memory 가능 여부, duration 을 반환합니다.
...
이제 memory recall 은 같은 `memory.sqlite` 안의 SQLite FTS5 인덱스를 사용합니다. 별도의 FAISS 나 임베딩 패키지는 필요하지 않습니다.

현재 Python 런타임이 이를 지원하는지 확인하려면 다음을 다시 실행하세요.
